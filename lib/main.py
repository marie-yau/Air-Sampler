...
def update_schedules_and_configuration(usb, ID):
    ...
    schedules_for_sampler = SamplerSchedule(path_to_schedule_file, configuration.get_pump_starts_before(),
                                            configuration.get_pump_stops_after(), configuration.get_pump_time_off_tolerance())
    # apply configuration to `Sampler` and `Diode` objects
    sampler = Sampler(configuration.get_pump_pin_number(), configuration.get_bag_numbers_to_valve_pin_numbers_dict(),
                      configuration.get_numbering_mode())
    diode = Diode(configuration.get_diode_pin_number(), configuration.get_numbering_mode())
...
logging.basicConfig(filename=current_time + ".log", format='%(asctime)s %(message)s', filemode='w')
...
while True:
    if usb.is_inserted():
        valves_schedule, pump_schedule, sampler, diode = update_schedules_and_configuration(usb, ID_number)
